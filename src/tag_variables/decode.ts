...
function decodeScopes(encodedScopes: string, names: string[]): ScopeInfo {
  ...
  for (const item of decodeScopeItem(encodedScopes)) {
    if (item.tag === Tag.ORIGINAL) {
      ...
      (stack.at(-1) as Scope).variables = variables;
    } else if (
      ...
      const startItem = rangeToStartItem.get(stack.at(-1) as Range);
...
function* decodeScopeItem(encodedScopes: string): Generator<Item> {
  ...
  let itemCount = 0;
  let lastGeneratedItem: GeneratedItem | undefined = undefined;
  ...
  while (iter.hasNext()) {
    ...
    if (tag === Tag.ORIGINAL) {
      ...
      for (let i = 0; i < variableCount; ++i) {
        item.variables.push(iter.nextUnsignedVLQ());
      }
      // Consume the empty tag here because this can never have children (in this impl).
      iter.nextUnsignedVLQ();
      ...
      for (let i = 0; i < bindingsCount; ++i) {
        const bindings: BindingItem["bindings"][number] = [];
        item.bindings.push(bindings);
        ...
        if (idxOrSubrangeCount >= -1) {
          ...
          bindings.push({
            line: lastGeneratedItem?.startLine ?? 0,
            column: lastGeneratedItem?.startColumn ?? 0,
